...
const Cours = () => {
  ...
  const categories = ['Tous', 'Scientifique', 'Langues', 'Littéraire', 'Humanités'];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-20 text-center">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold mb-4">Nos Cours de Soutien</h1>
          <p className="text-lg mb-6">Trouvez le professeur idéal pour progresser dans toutes les matières</p>
      ...
      {featuredCourses.length > 0 && (
        <section className="container mx-auto px-4 py-8">
          <h2 className="text-2xl font-bold mb-6 text-blue-700">Cours à la Une</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredCourses.map(course => (
              <div key={course.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col h-full border-2 border-yellow-300">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex justify-between items-start mb-2">
                    <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded">À la Une</span>
                  ...
                  <div className="flex justify-between items-center mt-auto">
                    ...
                    <Link to="/details" className="text-blue-600 hover:text-[#7ED321] font-medium flex items-center">
      ...
      <section className="container mx-auto px-4 pb-20">
        ...
        {filteredCourses.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.map(course => (
              <div key={course.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col h-full">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex justify-between items-start mb-2">
                    <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">{course.category}</span>
                    <div className="flex items-center">
                      <FaStar className="text-yellow-500 mr-1" />
                      <span className="text-gray-800 font-medium">{course.rating}</span>
                    </div>
          ...
          <p className="text-gray-600 mt-8">Aucun cours trouvé pour votre recherche.</p>
        )}
      </section>
    </div>
  );
