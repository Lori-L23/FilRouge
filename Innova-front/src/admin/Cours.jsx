...
const Cours = () => {
  ...
  const [coursPerPage] = useState(10);
  ...
  const fetchCours = async () => {
    try {
      ...
      const response = await Api.get("/api/data/cours", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setCours(response.data.data || response.data);
    } catch (error) {
  ...
  const filteredCours = cours.filter((cour) => {
    // Gérer le cas où matiere est un objet ou une chaîne
    const matiereText = typeof cour.matiere === 'object' && cour.matiere?.nom 
      ? cour.matiere.nom 
      : cour.matiere || '';
    ...
    const matchesSearch = 
      ...
      matiereText.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesNiveau = !niveauFilter || 
      cour.niveau_scolaire?.toLowerCase().includes(niveauFilter.toLowerCase());
    
    return matchesSearch && matchesNiveau;
  });

  // Pagination
  const indexOfLastCour = currentPage * coursPerPage;
  const indexOfFirstCour = indexOfLastCour - coursPerPage;
  const currentCours = filteredCours.slice(indexOfFirstCour, indexOfLastCour);
  const totalPages = Math.ceil(filteredCours.length / coursPerPage);
  ...
  const getStatutBadge = (statut) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statuts[statut] || "bg-gray-100 text-gray-800"}`}>
        {statut}
      </span>
    );
  };

  // Fonction helper pour extraire le nom de la matière
  const getMatiereNom = (matiere) => {
    if (typeof matiere === 'object' && matiere?.nom) {
      return matiere.nom;
    }
    return matiere || 'Non défini';
  };
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50">
          <div className="flex flex-col space-y-6">
            ...
            <div className="bg-white shadow-sm rounded-lg overflow-hidden">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentCours.length === 0 ? (
                      ...
                      currentCours.map((cour) => (
                        <tr key={cour.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {getMatiereNom(cour.matiere)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {getNiveauBadge(cour.niveau_scolaire)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {cour.tarif_horaire} FCFA/h
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {getStatutBadge(cour.statut)}
                          </td>
              ...
              {/* Pagination */}
              {filteredCours.length > coursPerPage && (
                  ...
                  <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                      <p className="text-sm text-gray-700">
                        Affichage de <span className="font-medium">{indexOfFirstCour + 1}</span> à{" "}
                        <span className="font-medium">
                          {Math.min(indexOfLastCour, filteredCours.length)}
                        </span>{" "}
                        sur <span className="font-medium">{filteredCours.length}</span> résultats
                      </p>
                    </div>
                    <div>
